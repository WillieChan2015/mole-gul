...
description: >
  Project-level skill for mole-gui: Analyze git commits between the latest tag and HEAD,
  ...
  within the mole-gui project. Also trigger when the user mentions version bumps, release preparation,
  ...
  Note: This is a project-specific skill located at `.claude/skills/changelog-generator/` — it will only
  trigger when working within the mole-gui project.
---
...
git diff <latest-tag>..HEAD --stat -- ':!README.md' ':!package.json' ':!pnpm-lock.yaml'
...
- A new file in `src/` or `src-tauri/src/` → **Added** (the capability it provides)
- A new test file → don't list separately; it validates a change already listed
- A renamed file → **Changed**
...
**Important:** Don't just parrot commit messages. A single commit may contain multiple distinct changes. Decompose and list each as its own entry.
...
- Describe the *user-visible* change, not the implementation detail
  - Good: "新增自动刷新功能，默认每 5 秒刷新一次" / "Added auto-refresh feature, refreshing every 5 seconds by default"
  - Bad: "新增 `useAutoRefresh.ts` 文件" / "Added `useAutoRefresh.ts` file"
- For technical changes that affect developers: mention the file or module name
  - "重构 Clean 模块，添加进度事件支持" / "Refactored Clean module, added progress event support"
...
- Don't list changes to `README.md`, `package.json`, or `pnpm-lock.yaml` — version bumps, keyword additions, badge updates, and README rewrites are metadata that reflect changes already captured elsewhere
