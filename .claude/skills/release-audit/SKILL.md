---
name: release-audit
description: 'Use when auditing, reviewing, or dry-running mole-gui release readiness without mutating repository state. Trigger whenever the user wants a preflight checklist, changelog/version/tag alignment review, or asks what is missing before release.'
argument-hint: '审计目标或关注点，例如 0.4.0、beta 发布检查、发版前还差什么'
user-invocable: true
---

# mole-gui Release Audit
...
- `cargo build`（因为会写入 `target/`，不属于严格只读）
...
3. **执行只读校验**
   - 可以运行：
     ...
     - `git status --short`
     - `git tag --list`
   - **敏感信息检查**：扫描源码和配置文件，确认不存在真实密钥、token 或其他敏感信息泄露。检查项包括：
     - 硬编码的 API Key（如 `sk-` 开头的真实密钥、`eyJ` 开头的 JWT）
     - 私钥（`-----BEGIN PRIVATE KEY`）
     - `.env` 文件误入版本控制
     - `dist/` 或 `src-tauri/target/` 中是否意外包含了源码或配置文件
   - 不要运行会写出构建产物或创建 tag/commit 的命令。

4. **识别阻塞项**
   - 常见阻塞包括：
     - `package.json` 和 `src-tauri/tauri.conf.json` 中的版本号不一致；
